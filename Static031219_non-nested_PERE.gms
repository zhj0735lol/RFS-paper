...
*Calculated from blendrate equation using the base model RFS scenario
          total            BBD             CELL
T1        0.024580        0.003908
T2        0.028257        0.003552
T3        0.029067        0.006136        0.000034
T4        0.063323        0.007717        0.000032
T5        0.066406        0.006324        0.000035
T6        0.069762        0.007928        0.000046
T7        0.073186        0.010182        0.000032
T8        0.072382        0.013003        0.000176
T9        0.075081        0.013867        0.000657
T10        0.077666        0.015265        0.001232
T11        0.080810        0.016162        0.001675
T12        0.081099        0.017031        0.001557
T13        0.081385        0.017091        0.002268
T14        0.081589        0.016318        0.001692
T15        0.081930        0.017205        0.001573
T16        0.082298        0.017283        0.002293
T17        0.082810        0.020123        0.003075
T18        0.083191        0.020382        0.003511
T19        0.083556        0.020471        0.003949
T20        0.083944        0.020566        0.004393
T21        0.084307        0.020655        0.004839
T22        0.084439        0.020688        0.005275
T23        0.084829        0.020783        0.005729
T24        0.085210        0.020876        0.006186
T25        0.085557        0.020961        0.006645
;

parameter blendrate_expost(T,*);
$Gdxin static031219_NONnested_test
...
RFS_RMANDATE2(T)$(ord(T) le card(t2))..              DCODE(T,'D6') =g=Blendrate(T,'total')*(Gassupply(t)+DSLSUPPLY(t,'pdsl'));              //
...
*RFS_QMANDATE2(T)$(ord(T) le card(t2))..             DCODE(T,'D6')   =g=  ( Qmandate(T,'total')-(Qmandate(T,'advanced')));//-(cellMandate(T)- CELLMandate_RM(T));             //
*RFS_qMANDATE2_bbd(T)$(ord(T) le card(t2))..          DCODE_out(T,'d4')=g=  Qmandate(T,'BBD');   //(1/(1+disc))**(Ord(T)-1)*
*RFS_qMANDATE2_cell(T)$(ord(T) le card(t2))..          (DCODE_out(T,'d3')+cellwaiver(t))=g= Qmandate(T,'CELL');      //(1/(1+disc))**(Ord(T)-1)*
...
blendrate_expost(T2,'total')$(EndFuel(t2,'gas')+ EndFuel(t2,'DIESEL')) =  ( Qmandate(T2,'total')-Qmandate(T2,'advanced'))/ (EndFuel(t2,'gas')+ EndFuel(t2,'DIESEL'));                        //
...
EndSS("Blender_Cost")=  Sum(T$(ord(T) EQ 11 ),       - GAS_BALANCE.m(t)* EndGas(t,'total')- Eth_BALANCE.m(T)*EndEth(t,'TOTAL') -DSL_balance.m(t)*DSLSUPPLY.l(t,'bdsl')-DSL_balance.m(t) *DSLSUPPLY.l(t,'Pdsl') );
...
calibrate('ethanol production')=  ( EndEth('t11','TOTAL')+ cellethSUPPLY.L('t11') )/1000;
...
execute_unload  'static042319_NONnested_test_GHG' ;
