...
$INCLUDE VMTGAS_2007_fuelDemand_PERE_0320.gms //VMTGAS_2007_fuelDemand_pere.gms
...
*Calculated from blendrate equation using the base model RFS scenario
          total            BBD             CELL
T1        0.028464        0.003905
T2        0.031809        0.003552
T3        0.035238        0.006136        0.000034
T4        0.070960        0.007705        0.000032
T5        0.072673        0.006316        0.000035
T6        0.077519        0.007905        0.000046
T7        0.083192        0.010157        0.000032
T8        0.085333        0.012969        0.000175
T9        0.089353        0.013828        0.000655
T10        0.093958        0.015232        0.001229
T11        0.098329        0.016110        0.001670
T12        0.099348        0.016973        0.001552
T13        0.100401        0.017033        0.002260
T14        0.099274        0.016265        0.001686
T15        0.100363        0.017146        0.001568
T16        0.101525        0.017223        0.002286
T17        0.105585        0.020043        0.003063
T18        0.106652        0.020300        0.003497
T19        0.107540        0.020388        0.003933
T20        0.108461        0.020483        0.004375
T21        0.109241        0.020550        0.004815
T22        0.109952        0.020603        0.005253
T23        0.110885        0.020698        0.005705
T24        0.111812        0.020791        0.006161
T25        0.112698        0.020875        0.006618
...
$Gdxin static031219_nested_test
...
T13  2.1    0.418       4.92          19.92
;
Qmandate(T,category)=Qmandate(T,category)*1000 ;

Qmandate(T,'bbd')$(ord(t) gt 13)= Blend_bbd(T-12);
Qmandate(T,'cell')$(ord(t) gt 13)= CELLMandate_RM(T-12);
Qmandate(T,'total')$(ord(t) gt 13)=QethMandate(T);
Qmandate(T,'Advanced') $(ord(t) gt 13)= Qmandate(T,'total')-15000;
...
cellwaiverPrice(t)$(ord(t) gt 12)=1.96;
...
OBJ..                           WF =e= Sum(T$(ord(T) le card(t2) ),        //eq 11
...
RFS_RMANDATE2_cell(T)$(ord(T) le card(t2))..           (DCODE(T,'D3'))=g= Blendrate(T,'CELL')*(Gassupply(t)+DSLSUPPLY(t,'PDSL'));     //cellwaiver(t)+
...
EnPrice(t,"i_GAS_PRICE","PRO")= gas_intercept('ROW') +gas_slope('ROW')*sum((point), GAS_SUPWEIGH.l(T,'ROW',point)*Gaspoint('ROW',point))  ;
...
EndSS("PS_corneth")=    Sum(T$(ord(T)EQ 11 ),
                          EnPrice(T,'cornETH',"pro")*Ethsupply.l(t) -  sum(point, EthCostGrid(point,t)*Eth_SUPWEIGH.l(T,point) )   );

EndSS("PS_celleth")=    Sum(T$(ord(T) EQ 11 ),
                        EnPrice(T,'cellETH',"pro")*EndFuel(t,'cell')-sum(point, CELLEthCostGrid(point)*CELLEth_SUPWEIGH.l(T,point)) );
...
EndSS("PS_BBD")=    Sum(T$(ord(T) EQ 11 ), DSLSUPPLY.l(t,'bdsl')*EnPrice(t,'BBD',"pro")- Sum(NOGRID,BBDPS_grid(nogrid,t) *BDSL_SUPWEIGH.l(t,nogrid))  );


EndSS("Blender")=  Sum(T$(ord(T) EQ 11 ),
                                   SUM(ethblend,SUM(VEHICLE,BlendedF.L(T,vehicle,ethblend))*ENPrice(t,ethblend,"CONS"))+ sum(diesel,DSLSUPPLY.L(t,diesel))*EnPrice(t,'DIESEL',"CONS")
                                    + DCODE.L(T,'d6')*RIN6.M(T) + DCODE.L(T,'d3')*RIN3.M(T)   + DCODE.L(T,'d4')*RIN4.M(T)
                                 - ENPrice(t,'GAS',"pro")* EndGas(t,'total')- ENPrice(t,'eth',"pro")*EndEth(t,'TOTAL') - EnPrice(t,'BBD',"pro")*DSLSUPPLY.l(t,'bdsl')-EnPrice(t,'DIESEL',"pro") *DSLSUPPLY.l(t,'Pdsl')
                                   -sum((ethblend,vehicle),blendcost(ethblend)*BlendedF.l(T,vehicle,ethblend))
...
output2007('gas V')= EndGas('t1','total');
output2007('eth V')= Endeth('t1','total');
output2007('gas P')=enPrice('t1','gas',"pro");
output2007('eth P')=enPrice('t1','eth',"pro");
*output2007('CVS stock')=EndNewstock('t1','cvs');
*output2007('ffv stock')=EndNewstock('t1','ffv');
*output2007('CVS VMT')=EndVMT('t1',"CVS");
*output2007('ffv VMT')=EndVMT('t1',"FFV");
...
*calibrate('cwc credit')= EndFuel('t1','cellwaiver') ;
...
OutputInterest('blenders_B_elas')=1/BBD_slope('t11')* ENPrice('t11','DIESEL',"pro")/EndFuel('t11','BBD');
OutputInterest('blenders_B_dTC')=(1/OutputInterest('blenders_B_elas')+1)* EnPrice('t11','BBD',"pro");
OutputInterest('blenders_E_elas')=1/Eth_slope('t11')* ENPrice('t11','corneth',"pro")/EndFuel('t11','CORNETH');
OutputInterest('blenders_E_dTC')=(1/OutputInterest('blenders_E_elas')+1)* EnPrice('t11','cornETH',"pro") ;


$include GHG.gms

execute_unload  'static032019_nested_TEST_ghg' ;
